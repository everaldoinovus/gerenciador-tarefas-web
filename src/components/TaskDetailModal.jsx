...
function TaskDetailModal({ isOpen, onRequestClose, task, sectors, onUpdateTask, onDeleteTask }) {
    ...
    // Este useEffect agora é a única fonte da verdade para o estado 'editableTask'
    useEffect(() => {
        if (task) {
    ...
    }, [task]); // Ele roda sempre que a prop 'task' de fora mudar

    // Este useEffect busca os dados de suporte (histórico, membros) quando o modal abre
    useEffect(() => {
        const fetchDetails = async () => {
            if (task) {
                try {
                    ...
                    onRequestClose(); // Fecha o modal se não conseguir carregar os detalhes
                }
            }
        };
        ...
        if (isOpen) {
            fetchDetails();
            setIsEditing(false); // Garante que o modal sempre abra no modo de visualização
        }
    }, [isOpen, task?.id]); // Roda quando o modal abre ou a tarefa selecionada muda
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            // Espera a atualização e o re-fetch dos dados no DashboardPage terminarem
            ...
            // O modal permanece aberto em modo de edição para o usuário ver os dados e tentar novamente
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleCancelEdit = () => {
        // Restaura o estado para a última 'prop' recebida
        const formattedDate = task.data_prevista_conclusao ? new Date(task.data_prevista_conclusao).toISOString().split('T')[0] : '';
        setEditableTask({ ...task, data_prevista_conclusao: formattedDate });
        setIsEditing(false);
    };
            ...
            {isEditing ? (
                <div className="task-modal-edit-mode">
                    ...
                    <div className="form-grid">
                        ...
                        <label>Status</label><select name="status" value={editableTask.status} onChange={handleChange}><option value="Pendente">Pendente</option><option value="Em Andamento">Em Andamento</option><option value="Concluída">Concluída</option></select>
                    ...
                    <div className="history-section">
                        ...
                        <ul className="history-list">{history.map((entry, index) => {
                            ...
                            return (
                                <li key={index}>
                                    <div className="history-entry">
                                        <span>Movido para <strong>{entry.status_novo}</strong> por <em>{entry.usuario_alteracao_email}</em></span>
                                        <span className="history-date">{new Date(entry.data_alteracao).toLocaleString('pt-BR')}</span>
                                    </div>
                                    {entry.status_anterior && <div className="history-duration">Tempo em '{entry.status_anterior}': <strong>{timeInPreviousStatus}</strong></div>}
                                </li>
                            );
                        })}</ul>
                    </div>
