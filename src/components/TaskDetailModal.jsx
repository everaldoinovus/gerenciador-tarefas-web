...
function TaskDetailModal({ isOpen, onRequestClose, task, sectors, onUpdateTask, onDeleteTask }) {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            if (task) {
                ...
                } catch (error) { toast.error("Não foi possível carregar os detalhes da tarefa."); }
            }
        };
    ...
    const handleSaveChanges = () => { onUpdateTask(editableTask.id, editableTask); setIsEditing(false); };
                    ...
                    <div className="history-section">
                        ...
                        <ul className="history-list">
                            {history.map((entry, index) => {
                                ...
                                return (
                                    <li key={index}>
                                        <div className="history-entry"><span>Movido para <strong>{entry.status_novo}</strong> por <em>{entry.usuario_alteracao_email}</em></span><span className="history-date">{new Date(entry.data_alteracao).toLocaleString('pt-BR')}</span></div>
