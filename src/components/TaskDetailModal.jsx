...
// A prop onDeleteTask foi removida, pois a ação agora é arquivar
function TaskDetailModal({ isOpen, onRequestClose, task, sectors, statuses, onUpdateTask }) {
    ...
    // ===== FUNÇÃO DE DELETAR SUBSTITUÍDA PELA FUNÇÃO DE ARQUIVAR =====
    const handleArchive = () => {
        if (window.confirm("Tem certeza que deseja arquivar esta tarefa? Ela será removida do quadro principal.")) {
            api.put(`/tarefas/${task.id}/archive`)
                .then(() => {
                    toast.success("Tarefa arquivada com sucesso!");
                    onUpdateTask(); // onUpdateTask força o refresh de dados no Dashboard
                    onRequestClose(); // Fecha o modal
                })
                .catch(error => {
                    toast.error(error.response?.data?.error || "Erro ao arquivar a tarefa.");
                });
        }
    };
            ...
            {isEditing ? (
                ...
                <div className="task-modal-view-mode">
                    ...
                    <div className="modal-actions">
                        ...
                        {/* ===== BOTÃO DE DELETAR SUBSTITUÍDO PELO BOTÃO DE ARQUIVAR ===== */}
                        <button onClick={handleArchive} className="btn btn-secondary">Arquivar</button>
