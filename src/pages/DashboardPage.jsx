...
function DashboardPage() {
    ...
    const handleAddTask = async (taskData) => { try { await api.post('/tarefas', taskData); toast.success("Tarefa adicionada com sucesso!"); fetchTasks(); } catch (error) { toast.error("Erro ao adicionar tarefa."); } };
    const handleUpdateTask = async (taskId, updatedData) => { try { const originalTask = tasks.find(task => task.id === taskId); if (!originalTask) return; const finalData = { ...originalTask, ...updatedData }; await api.put(`/tarefas/${taskId}`, finalData); if (updatedData.status === originalTask.status || !updatedData.status) { toast.success("Tarefa atualizada com sucesso!"); } setTasks(tasks.map(task => task.id === taskId ? finalData : task)); if (selectedTask && selectedTask.id === taskId) { setSelectedTask(finalData); } } catch (error) { toast.error("Erro ao atualizar tarefa."); } };
    const handleUpdateTaskStatus = (taskId, newStatus) => { handleUpdateTask(taskId, { status: newStatus }); };
    const handleDeleteTask = async (taskId) => { try { await api.delete(`/tarefas/${taskId}`); toast.success("Tarefa deletada com sucesso!"); fetchTasks(); } catch (error) { toast.error("Erro ao deletar tarefa."); } };
